...
- All built-in Core rules are enabled by default except `Rule.Numbers`. This means the protected country, popular-city, celebrity, nationality, currency, religion, landmark, event, award, fictional-character, franchise, profession, and military rules are all active by default.
